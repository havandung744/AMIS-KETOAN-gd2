...
namespace MISA.Web04.Api.Controllers
{
    ...
    public class EmployeesController : MISABaseController<Employee>
    {
        ...
        /// </summary>
        /// <param name="employeeRepository">khởi tạo đối tượng employeeRepository</param>
        /// <param name="employeeService">khởi tạo đối tượng employeeService</param>
        ...
        public async Task<IActionResult> ExportExcel(int? pageSize, int? pageIndex, string? employeeFilter, string? bankName, int? gender, Guid? departmentId, bool IsOrganizations)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                foreach (var (employee, index) in employees.Select((employee, index) => (employee, index)))
                {
                    ...
                    worksheet.Cell(currentRow, 7).Value = employee.EmployeePosition;
                ...
                worksheet.Range($"E3:E{currentRow - 1}").Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
